...
function PANEL:buildSheet(target, isadmin, playerdata)

	local sheetdata = playerdata.charsheetinfo
	...
	charsheetstalkernetright:SetText(playerdata.repname)
	...
	for k, v in SortedPairsByMemberValue(ix.attributes.list, "name") do
		...
		charsheetattributesright:SetText(v.name..": "..playerdata.attributes[k])
	...
	for k, v in SortedPairsByMemberValue(ix.attributes.list, "name") do
		if v.secondary == true then
			...
			charsheetsecattributesright:SetText(v.name..": "..playerdata.attributes[k])
	...
	for k, v in SortedPairsByMemberValue(ix.perks.list, "name") do
		...
		charsheetperksright:SetText(v.name..": "..playerdata.perks[k])
	...
	if isadmin then
		...
		netstream.Start("ixDescriptionTargetSendText", target, sheet)
