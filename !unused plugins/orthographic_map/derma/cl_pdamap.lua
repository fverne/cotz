...
if ORTHOGRAPHIC then
    ...
    local MAP_TEXTURES = {
        ["rp_marsh_cs"] = {
            texture = Material("cotz/pda/marsh2.png"), -- https://i.imgur.com/ahbVrT1.jpg
            sizeX = 1922,
            sizeY = 1858,
        },
    }
    local contW, contH = 1245, 690
    local scrollspeed = 0.03
    local isClicking = false
    local xOffset, yOffset = 0, 0
    local curX, curY = 0, 0 
    local lastX, lastY
    local mapText = Material("cotz/pda_maps/nomap.png")
    local mapOrgW, mapOrgH = 1920, 1857
    ...
    function PANEL:Init()
        hook.Add("ShouldDrawCrosshair", "pdaMapHideCrosshair", function() return false end)
        self:SetTitle("")
        self:SetSize(1245, 690)
        self:SetPos(ScrW() / 2 - 645, ScrH() / 2 - 352)
        self:MakePopup()
        self:ShowCloseButton(false)
        self:MoveToFront()
        self.delayTime = 0
        ...
        self.map.Paint = function(pnl, w, h) end
        self:onDisplay()
    ...
    function PANEL:Paint(w, h)
        ...
        draw.SimpleText("PRESS [TAB] TO CLOSE", "stalkerregulartitlefont", 42, h - 60, Color(255, 255, 255), TEXT_ALIGN_LEFT)
    ...
    function PANEL:OnMouseWheeled(delta)
        ...
        mapW, mapH = math.Clamp(mapOrgW*zoomOffset, contW, mapOrgW), math.Clamp(mapOrgH*zoomOffset, contW*0.9, mapOrgW + 200)
        print(mapW, mapH)
