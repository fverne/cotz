...
DIALOGUE.addTopic("AboutProgression", {
	...
	ShouldAdd = function()
		return #ix.progression.GetActiveProgressions("'Haggler'") > 0
	end,
})
		...
		if( CLIENT ) then
			...
			if #barters <= 0 then
				...
				for _, barter in pairs(barters) do
					self.response = self.response.."\n    "..ix.npcbarter.barterdict["'Haggler'"][barter].description
				end
			end
	...
	GetDynamicOptions = function(self, client, target)
		...
		for _, barterid in pairs(ix.npcbarter.GetActiveBartersForNPC("'Haggler'")) do
			...
			for _, reqitem in pairs(barterstruct.reqItem) do
				...
				table.insert(dynopts, {statement = reqItemCnt.."x "..ix.item.list[reqitem[1]].name.." to "..barterCnt.."x "..ix.item.list[barterItem[1]].name, topicID = "StartBarter", dyndata = {npcname = "'Haggler'", identifier = barterid, reqitem = reqitem[1]}})
			end
		end
