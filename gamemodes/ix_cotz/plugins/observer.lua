...
if (CLIENT) then
	...
	ix.option.Add("observerESP - NPCs", ix.type.bool, true, {
		category = "observer",
		hidden = function()
			return !CAMI.PlayerHasAccess(LocalPlayer(), "Helix - Observer", nil)
		end
	...
	ix.option.Add("observerESP - Items", ix.type.bool, true, {
		category = "observer",
		hidden = function()
			return !CAMI.PlayerHasAccess(LocalPlayer(), "Helix - Observer", nil)
		end
	...
	ix.option.Add("observerESP - Shipment", ix.type.bool, true, {
		category = "observer",
		hidden = function()
			return !CAMI.PlayerHasAccess(LocalPlayer(), "Helix - Observer", nil)
		end
	...
	function PLUGIN:HUDPaint()
		...
		if (ix.option.Get("observerESP", true) and client:GetMoveType() == MOVETYPE_NOCLIP and
			...
			for k, v in ipairs(ents.GetAll()) do
				...
				screenPosition = v:GetPos():ToScreen()

				marginX, marginY = scrH * .1, scrH * .1
				...
				if (ix.option.Get("observerESP - NPCs", true) and v:IsNPC()) then
					surface.SetDrawColor(255, 0, 0, alpha)
					surface.DrawRect(x - size/2, y - size/2, size, size)	
					if IsValid(v) then
						ix.util.DrawText(v:GetClass(), x, y - size, ColorAlpha(Color(255, 0, 0), alpha, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, nil, alpha))
					end				
				end

				if (ix.option.Get("observerESP - Items", true) and v:GetClass() == "ix_item") then
					surface.SetDrawColor(255, 0, 255, alpha)
					surface.DrawRect(x - size/2, y - size/2, size, size)	
					if IsValid(v) then
						ix.util.DrawText(v:GetItemTable().name, x, y - size, ColorAlpha(Color(255, 0, 255), alpha, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, nil, alpha))
					end				
				end

				if (ix.option.Get("observerESP - Shipments", true) and v:GetClass() == "ix_shipment") then
					surface.SetDrawColor(255, 0, 255, alpha)
					surface.DrawRect(x - size/2, y - size/2, size, size)	
					if IsValid(v) then
