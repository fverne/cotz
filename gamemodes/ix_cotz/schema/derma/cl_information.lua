...
function PANEL:Init()
	...
	self.name:SetPos(parent:GetWide()*0.64, parent:GetTall()*0.042)
	...
	if (!suppress.name) then
		...
		namePanel:SetTall(self:GetParent():GetTall()*0.1)
	...
	/*	self.characterInfo = self:Add("Panel")
		...
		if (!suppress.money) then
			...
			self.money:SetPos(parent:GetWide()*0.64, parent:GetTall()*0.108)
