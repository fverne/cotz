local PLUGIN = PLUGIN
PLUGIN.name = "Artifact Sale"
PLUGIN.author = "gumlefar"
PLUGIN.desc = "Artifact spot sale plugin"
...
if (CLIENT) then
	netstream.Hook("saleArtOpn", function(client, entity, artSales)
		...
		if IsValid(entity) then
			...
			for k, v in ipairs(artSales[1]) do
				...
				btn:Dock(TOP)
				btn:SetText(ix.item.list[v].name.." - "..ix.item.list[v].price*ix.plugin.list["artifactsale"].priceModifier)
				btn:DockMargin(5, 5, 5, 0)

				function btn.DoClick()
					local arti = k
				
					netstream.Start("saleArt", arti, true)
					frame:Close()
				end
			end
			...
			for k, v in ipairs(artSales[2]) do
				...
				function btn.DoClick()
					...
					netstream.Start("saleArt", arti, false)
					frame:Close()
				end
			end
	...
	netstream.Hook("saleArt", function(client, artID, isDaily)
		ix.plugin.list["artifactsale"]:sellArtifact(client, artID, isDaily)
	end)
	...
	function PLUGIN:LoadData()
		...
		for k, v in ipairs(data.persistList or {}) do
			local entity = ents.Create("ix_artifactsale")
			entity:SetPos(v.pos)
	...
	function PLUGIN:SaveData()
		...
		local data = {}
		for k, v in ipairs(ents.FindByClass("ix_artifactsale")) do
			data[#data + 1] = {
				pos = v:GetPos(),
				angles = v:GetAngles(),
			}
...
ix.command.Add("artifactsalegetnew", {
	adminOnly = true,
...
ix.command.Add("artifactsaleprintcurrent", {
	adminOnly = true,
	OnRun = function(self, client, arguments)
