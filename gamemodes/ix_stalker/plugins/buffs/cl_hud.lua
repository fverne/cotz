...
function PLUGIN:HUDPaint()
	...
	if LocalPlayer():HasBuff("buff_slowheal") then
		...
		surface.DrawTexturedRect(ScrW()*0.82, ScrH()*0.75, ScrW()*0.018, ScrH()*0.032)
	end

	if LocalPlayer():HasBuff("buff_staminarestore") then
		surface.SetDrawColor(Color(255, 255, 255, 255))
		surface.SetMaterial(Texture2)
		surface.DrawTexturedRect(ScrW()*0.84, ScrH()*0.75, ScrW()*0.018, ScrH()*0.032)
	end
	
	if LocalPlayer():HasBuff("buff_radiationremoval") then
		surface.SetDrawColor(Color(255, 255, 255, 255))
		surface.SetMaterial(Texture3)
		surface.DrawTexturedRect(ScrW()*0.86, ScrH()*0.75, ScrW()*0.018, ScrH()*0.032)
	end

	if LocalPlayer():HasBuff("buff_radprotect") then
		surface.SetDrawColor(Color(255, 255, 255, 255))
		surface.SetMaterial(Texture4)
		surface.DrawTexturedRect(ScrW()*0.88, ScrH()*0.75, ScrW()*0.018, ScrH()*0.032)
	end

	if LocalPlayer():HasBuff("debuff_radiation") then
		surface.SetDrawColor(Color(255, 0, 0, 255))
		surface.SetMaterial(Texture5)
		surface.DrawTexturedRect(ScrW()*0.9, ScrH()*0.75, ScrW()*0.018, ScrH()*0.032)
	end
