...
function PANEL:Init()
	...
	self.custominv = self:Add("ixStalkerInventoryPanel")
	--ix.gui.inv1.bNoBackgroundBlur = true
	self.custominv:ShowCloseButton(true)
	self.custominv.Close = function(this)
		net.Start("ixStorageClose")
		net.SendToServer()
		self:Remove()
	end
...
function PANEL:SetLocalInventory(inventory)
	if (IsValid(ix.gui.inv1) and !IsValid(ix.gui.menu)) then
		...
		self.custominv:SetPos(self:GetWide() / 2 + self:GetFrameMargin() / 2, self:GetTall() / 2 - ix.gui.inv1:GetTall() / 2)
	end
end
