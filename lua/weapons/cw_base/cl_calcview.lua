...
SWEP.autoCenterExclusions = { -- if the weapon's state is any of this, then we will force-auto-center if it is enabled
	[CW_RUNNING] = true,
	[CW_ACTION] = true,
...
local cw_freeaim_autocenter = GetConVar("cw_freeaim_autocenter")
local cw_freeaim_autocenter_aim = GetConVar("cw_freeaim_autocenter_aim")
local cw_freeaim_lazyaim = GetConVar("cw_freeaim_lazyaim")

local cw_freeaim_yawlimit = GetConVar("cw_freeaim_pitchlimit")
local cw_freeaim_pitchlimit = GetConVar("cw_freeaim_pitchlimit")

local cw_freeaim_autocenter_time = GetConVar("cw_freeaim_autocenter_time")
local cw_freeaim_center_mouse_impendance = GetConVar("cw_freeaim_center_mouse_impendance")
...
function SWEP:getFreeAimDotToCenter()
	...
	return dist / (cw_freeaim_yawlimit:GetInt() + cw_freeaim_pitchlimit:GetInt())
end
...
function SWEP:CalcView(ply, pos, ang, fov)
	...
	self.autoCenterFreeAim = cw_freeaim_autocenter:GetInt() > 0
...
--[[
	...
	if self.freeAimOn and (not self.forceFreeAimOffTime or CurTime() < self.forceFreeAimOffTime) then
		...
		if self.shouldUpdateAngles then
			...
			local lazyAim = cw_freeaim_lazyaim:GetInt()
		...
		if self.autoCenterFreeAim then
			if self.mouseActive then
				self.lastMouseActivity = CurTime() + cw_freeaim_autocenter_time:GetInt()
			end

			local canAutoCenter = CurTime() > self.lastMouseActivity
			local shouldAutoCenter = false
			local aimAutoCenter = cw_freeaim_autocenter_aim:GetInt() > 0
		...
		local yawLimit = cw_freeaim_yawlimit:GetInt()
		local pitchLimit = cw_freeaim_pitchlimit:GetInt()
...
function SWEP.CreateMove(move)
	...
	if IsValid(wep) and wep.CW20Weapon then
		...
		if wep.dt.BipodDeployed and wep.DeployAngle then
			...
			local dif = math.AngleDifference(EA.y, wep.DeployAngle.y)
...
function SWEP:AdjustMouseSensitivity()
	...
	if self.freeAimOn and not self.dt.BipodDeployed then
		...
		local mouseImpendance = cw_freeaim_center_mouse_impendance:GetInt()
