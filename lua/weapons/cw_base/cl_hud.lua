...
local cw_crosshair = GetConVar("cw_crosshair")
...
function SWEP:DrawHUD()
	...
	if not disableCrosshair then
		if self.CrosshairEnabled and cw_crosshair:GetInt() > 0 then
			...
			self.CrossAmount = LerpCW20(FT * 10, self.CrossAmount, (self.CurCone * 350) * (90 / (math.Clamp(GetConVar("fov_desired"):GetInt(), 75, 90) - self.CurFOVMod)))
	...
	if not disableCustomHUD then
		...
		if not customHUD and cw_customhud_ammo:GetInt() <= 0 then
...
function SWEP:drawCustomHUD()
	...
	if cw_customhud_ammo:GetInt() <= 0 then
		...
		if reloadProgress then
			draw.ShadowText("RELOADING  " .. reloadProgress .. "%", cwhud32, baseXAMMO + 15, baseYAMMO, self.HUD_MagTextColor, self.HUDColors.black, 1, TEXT_ALIGN_RIGHT, TEXT_ALIGN_CENTER)
		else
			draw.ShadowText(self.HUD_MagText .. self:getMagCapacity(), cwhud32, baseXAMMO + 15, baseYAMMO, self.HUD_MagTextColor, self.HUDColors.black, 1, TEXT_ALIGN_RIGHT, TEXT_ALIGN_CENTER)
		end
