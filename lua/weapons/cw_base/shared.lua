...
-- luacheck: globals CW_IDLE CW_RUNNING CW_AIMING CW_ACTION CW_CUSTOMIZE
-- luacheck: globals CW_HOLSTER_START CW_HOLSTER_END CW_PRONE_BUSY CW_PRONE_MOVING
CW_IDLE = 0
CW_RUNNING = 1
CW_AIMING = 2
CW_ACTION = 3
...
SWEP.ToggleM203States = {
	[CW_IDLE] = true,
	[CW_RUNNING] = true
}

SWEP.M203ReloadSounds = {
	[1] = {time = 0.3, sound = "CW_M203_OPEN"},
...
function SWEP:Initialize()
	...
	if CLIENT then
		if not self.ScopeRT then
			...
			base:initRenderTarget(CustomizableWeaponry:getRenderTargetSize(GetConVar("cw_rt_scope_quality"):GetInt()))
...
function SWEP:SetupDataTables()
	...
	self:NetworkVar("Bool", 1, "BipodDeployed")
	self:NetworkVar("Bool", 2, "M203Active")
...
function SWEP:Deploy()
	...
	if (SP and SERVER) then
		net.Start("CW20_DEPLOY")
		net.Send(self.Owner)
	end

	if self.dt.M203Active then
		if SERVER and SP then
			net.Start("CW20_M203OFF")
			net.Send(self.Owner)
		end
...
function SWEP:Think()
	...
	if CT > self.GlobalDelay then
		local wl = self.Owner:WaterLevel()
...
function SWEP:PrimaryAttack()
	...
	if IsFirstTimePredicted() then
		...
		self:EmitSound(self:getFireSound(), 105, 100)
...
function SWEP:MakeRecoil(mod)
	mod = self:GetRecoilModifier(mod)
...
if CLIENT then
	function SWEP:ViewModelDrawn()
		local EP, EA2, FT = EyePos(), EyeAngles(), FrameTime()
		...
		if IsValid(ply) then
			...
			if IsValid(ply) and IsValid(wep) and wep.CW20Weapon then
				if CustomizableWeaponry.firemodes.registeredByID[Mode] then
					local t = CustomizableWeaponry.firemodes.registeredByID[Mode]
					...
					if ply == LocalPlayer() then
						...
						CustomizableWeaponry.callbacks.processCategory(wep, "fireModeChanged")
					end
				end
			end
		end
